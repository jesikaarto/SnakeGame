...
/**
 * A class that represent a panel container for the main board
 * 
 * @author Jes
 */
public class GameBoardPanel extends JPanel implements ActionListener{
    ...
    public void drawSnake(Graphics2D g2) {
        for (int i = 0; i < snake.getLength(); i++) {
            if (i == 0) 
            {
                ...
                g2.setColor(Color.black);
                g2.draw(snake.getBody().get(i)); //dikasi warna bordernya saja
            }
        }
    }
    ...
    public void drawStatusBar(Graphics2D g2) {
        ...
        if (isGameOver()) {
            ...
            g2.drawString("Game Over!", 440, 350);
    ...
    public boolean isBoundryCollisioned() {
        if (this.snake.getDirection() == 1) { //up-side
            double centerY = this.snake.getBody().get(0).getMinY();
            return centerY < 127;
        }
        else if (this.snake.getDirection() == 2) { //down-side
